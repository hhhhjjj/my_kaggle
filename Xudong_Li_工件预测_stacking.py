...
scale_features = [ '7_9', '8_9', '7_8', '8_10', '9_10', '7_10', '9%8', '7/8', '7%8', '8/9',
...
for i in tqdm(scale_features):
	for j in ['7_8', '7_9', '8_9']:
		if i != j:
			train_data['mean_{0}_{1}'.format(i, j)] = train_data_[i].map(train_data_.groupby(i)[j].nunique())
			test_data['mean_{0}_{1}'.format(i, j)] = test_data_[i].map(test_data_.groupby(i)[j].nunique())
			group_feat.append('mean_{0}_{1}'.format(i, j))
'''
for i in tqdm(['Parameter5', 'Parameter7', 'Parameter8', 'Parameter9', 'Parameter10']):
	for j in ['Parameter8', 'Parameter9']:
		if i != j:
			train_data['mean_{0}_{1}'.format(i, j)] = train_data_[i].map(train_data_.groupby(i)[j].nunique())
			test_data['mean_{0}_{1}'.format(i, j)] = test_data_[i].map(test_data_.groupby(i)[j].nunique())
			group_feat.append('mean_{0}_{1}'.format(i, j))
...
drop_feat = drop_feat + ['Parameter6', #'Parameter7', 'Parameter8', 'Parameter9', '7_8',
	#'8*8', , '7*7',
						 #'7_9', '8_9', '9_10',
						 #'5_6',
...
def catboost_model():
	cbt_model = cbt.CatBoostClassifier(#iterations=100000, learning_rate=0.01, verbose=0,
		iterations=1500, learning_rate=0.01, verbose=0,
...
def xgboost_importance():
	model = xgboost_model()
	model.fit(x_train, np.argmax(labels, 1))
	importance_df = pd.DataFrame({'column': features, 'importance': model.feature_importances_}).sort_values(by='importance')
	print('xgboost features importance:\n', importance_df)
	return importance_df['column'].values[::-1]
...
svc = SVC(C=0.01, probability=True)#LinearSVC(property)
